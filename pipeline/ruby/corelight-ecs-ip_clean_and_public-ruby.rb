...
def filter(event)
  ...
  if !clean_ip_addresses.empty?
    ...
    if number_of_ip_addresses == 1
      ...
      event.set("#{@parent_field}[ip_type]", ip_addresses_type[0])
      event.set("#{@parent_field}[ip_rfc]", ip_addresses_rfc[0])
      event.remove("#{@orig_ip_address}")
      event.set("#{@orig_ip_address}", clean_ip_addresses[0])

    else
      event.set("#{@parent_field}[ip_version]", version_ip_addresses)
      event.set("#{@parent_field}[is_ipv6]", ip_addresses_is_ipv6)
      event.set("#{@parent_field}[ip_public]", ip_addresses_public)
      event.set("#{@parent_field}[ip_type]", ip_addresses_type)
      event.set("#{@parent_field}[ip_rfc]", ip_addresses_rfc)
      event.remove("#{@orig_ip_address}")
